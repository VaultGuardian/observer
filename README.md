...
- Failed HTTP probes: `400`/`403`/`404`/`405`/`410` responses, independent of path or payload. A 404 on `/.env` is still a 404; the request shape alone never escapes this filter. Two things do bypass it, both of them actual disclosure evidence rather than scanner intent: **disclosure in the log line itself** (e.g., a credential dump in the response body), and **a captured response body that deterministically discloses sensitive data** (passwd/dotenv/PEM) even though the line logged a 4xx. The second check is a best-effort one-shot REC lookup; when REC is disabled or the capture has not landed yet, suppression proceeds as before. Body-parser exploits that succeed (XXE, deserialization, Log4Shell) generate downstream log lines (process spawns, file reads, response disclosures) that the seed patterns, policy engine, and REC catch with confirmed-impact evidence rather than request-shape suspicion.
